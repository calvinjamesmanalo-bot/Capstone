                        ...
                        <td class="px-5 py-6 align-top">
                            <div class="flex items-center justify-end gap-3">
                                @if(in_array(auth()->user()->role, ['registrar', 'admin']) && $req->status === 'processed')
                                         <div class="flex w-full flex-col gap-3">
                                             ...
                                             @elseif(in_array(strtolower($req->document_type), ['form 137', 'form 138', 'f137', 'f138']))
                                                <a href="{{ rtrim(config('generator.url'), '/') }}/{{ str_contains(strtolower($req->document_type), '137') ? 'f137' : 'f138' }}/preview?{{ http_build_query(['student' => $req->student_number]) }}" target="_blank" class="px-4 py-2 bg-blue-50 text-blue-700 rounded-xl text-xs font-black uppercase tracking-widest hover:bg-blue-100 transition-all text-center border border-blue-100">
                                ...
                                @elseif(in_array(auth()->user()->role, ['records_officer', 'admin']))
                                    ...
                                    <a href="{{ rtrim(config('generator.url'), '/') }}/{{ str_contains(strtolower($req->document_type), '137') ? 'f137' : 'f138' }}/preview?{{ http_build_query(['student' => $req->student_number]) }}" target="_blank" class="w-12 h-12 flex items-center justify-center bg-blue-600 text-white rounded-xl shadow-lg hover:bg-blue-700 transition-all" title="Open {{ $req->document_type }} Maker">
                                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
