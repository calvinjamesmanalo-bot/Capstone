    ...
    private function parseExcelGrades($filePath)
    {
        ...
        try {
            ...
            for ($r = 1; $r <= 15; $r++) {
                ...
                if ($foundQ1) break; // Found the header row
            }
    ...
    private function generateSpreadsheet($student, $selectedUploadIds = [])
    {
        ...
        if (!empty($selectedUploadIds)) {
            ...
            foreach ($uploads as $upload) {
                $parsedGrades = $this->parseExcelGrades($upload->file_path);
                if (!empty($parsedGrades)) {
                    ...
                    $sheet->getStyle("A{$currentRow}")->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)->getStartColor()->setRGB('E0E0E0');
                    $currentRow++;
                    ...
                    foreach ($parsedGrades as $grade) {
                        ...
                        $sheet->getStyle("B{$currentRow}:F{$currentRow}")->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER);
                        $currentRow++;
                    }
                    $currentRow++;
                }
