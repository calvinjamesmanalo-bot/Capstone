...
<header class="border-b border-slate-200 bg-white"><div class="mx-auto flex max-w-5xl items-center justify-between px-5 py-5 sm:px-8">
    <a href="{{ route('home') }}" class="flex items-center gap-3"><span class="grid h-10 w-10 place-items-center rounded-xl bg-blue-600 text-sm font-bold text-white">SF</span><span><span class="block font-bold">School Forms Maker</span><span class="block text-xs text-slate-500">Student report card workspace</span></span></a>
    <a href="{{ route('students.index') }}" class="rounded-lg bg-blue-50 px-4 py-2 text-sm font-bold text-blue-700">Grade Sheet Records</a>
</div></header>
    ...
    <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        ...
        <div class="p-6 sm:p-8">
            ...
            <form id="f137-section" class="hidden" method="GET" action="{{ route('f137.preview') }}" target="_blank">
                ...
                <div class="mt-5 flex flex-wrap justify-end gap-3"><a href="{{ route('f137.template') }}" class="rounded-lg border border-slate-300 bg-white px-6 py-3 text-sm font-bold text-slate-700">Download blank template</a><button class="rounded-lg bg-blue-600 px-6 py-3 text-sm font-bold text-white">Preview generated F137</button></div>
            </form>
        </div>
    </div>
</main>
